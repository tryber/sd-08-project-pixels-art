window.onload = function () {
  ...
  // Adiciona cores nos pixels da paleta
  ...
  let multiplicador = 49;
  for (let index = 1; index < colorPixel.length; index += 1) {
    let randomColor = Math.trunc(Math.random() * multiplicador);
  ...
  document.getElementById("clear-board").addEventListener("click", function () {
    for (let index = 0; index < 25; index += 1) {
      document.querySelectorAll(".pixel")[index].style.backgroundColor =
        "rgb(255, 255, 255)";
    }
  });
};
